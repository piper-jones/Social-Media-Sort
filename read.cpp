...
vector<Post > readFile(vector<Post> &data) {
    string file_path = "C:\\Users\\dang1\\CLionProjects\\Social-Media-Sort\\social_media_engagement_data.csv";
    ...
    while (getline(file, line)) {
        ...
        try {
            ...
            shares = stoi(cell);
            getline(ss, cell, ',');
            impressions = stoi(cell);
            getline(ss, cell, ',');
            reach = stoi(cell);
            getline(ss, cell, ',');
            engagement_rate = stof(cell);
            getline(ss, cell, ',');
            audience_age = stoi(cell);
            ...
            data.push_back(Post(platform, post_id, post_type, post_content, post_timestamp, stof(temp), likes, comments,
                                shares, impressions, reach, engagement_rate, audience_age, audience_gender,
                                audience_location, audience_interests, sentiment));
